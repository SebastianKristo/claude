// Mock for kd-bil-card – gjenskaper designets eksempelverdier (Bil v3.dc.html).
(() => {
  ...
  const HIST = {
    ...
    [ODO]: [[day(mon, 8), 17137], [A[1], 17279], [B[1], 17297], [Cc[1], 17309], [D[1], 17321]],
  ...
  // «Daglig kjøring siste 7 dager» (designets KM = [34, 12, 14, 12, 58, 13, 4], i dag sist): kilometerstand ved dagslutt
  const ODO_END = [17174, 17208, 17220, 17234, 17246, 17304, 17317]; // for 7 … 1 dager siden
  ...
  MOCK.make = function () {
    ...
    h.callWS = (msg) => {
      ...
      if (msg.type === 'recorder/statistics_during_period' && (msg.statistic_ids || []).includes(ODO))
        return Promise.resolve({ [ODO]: ODO_END.map((v, i) => ({ start: day(7 - i, 0), end: day(6 - i, 0), state: v, max: v })) });
      return orig(msg);
    };
    return h;
  };
  ...
  MOCK.add({
    [BAT]: { state: '71', attributes: { unit_of_measurement: '%', device_class: 'battery' } },
    'sensor.tesla_model_y_batteri_estimert_batterirekkevidde': { state: '298.2', attributes: { unit_of_measurement: 'km' } },
    ...
    'sensor.ki_tesla_ladepris_estimat': { state: '3.7', attributes: { unit_of_measurement: 'kr' } },
    'sensor.ki_tesla_forrige_lading_kostnad': { state: '69', attributes: { unit_of_measurement: 'kr' } },
    ...
    [ODO]: { state: '17321', attributes: { unit_of_measurement: 'km' } },
    'sensor.tesla_model_y_energy_consumption': { state: '160', attributes: { unit_of_measurement: 'Wh/km' } },
